...
def simulate_session(bars: pd.DataFrame, band: pd.DataFrame,
    ...
    for i in range(n):
        ...
        if is_decision:
            ...
            # Cross-asset conditioning overlay (HYP-0005). No-op when cond_mode is
            # None (strict parity with the frozen baseline, asserted by the runner).
            # ext_state maps this session's decision tod -> contemporaneous ES
            # noise-VWAP state in {+1,-1,0}; missing tod defaults to 0 (ES flat/no info).
            if cond_mode is not None:
                es = ext_state.get(t, 0) if ext_state is not None else 0
                if cond_mode == "agree":       # take GC signal only if ES agrees
                    if want != es:
                        want = 0
                elif cond_mode == "gate":      # take GC signal only if ES in a breakout
                    if es == 0:
                        want = 0
                elif cond_mode == "es_dir":    # enter GC in ES's direction
                    want = es
                elif cond_mode == "es_opp":    # enter GC OPPOSITE to ES's direction
                    want = -es
                elif cond_mode == "disagree":  # take GC signal only if ES disagrees
...
def run(bars: pd.DataFrame, bands: pd.DataFrame,
    ...
    ext_state_by_date (HYP-0005): maps date -> {tod: es_state}. Only consulted when
