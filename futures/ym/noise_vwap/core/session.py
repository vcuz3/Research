"""
YM (E-mini Dow) session loader for the Noise-Area + VWAP momentum grid studies.
...
# RESEARCH_WORKFLOW; kept identical to core.data.DATA. parents[2] = futures/ym.
DATA = Path(__file__).resolve().parents[2] / "data"
PATHS = {"YM": DATA / "YM_1m_clean.parquet"}
...
if __name__ == "__main__":
    for anchor in ("rth", "eth"):
        d = load_rth("YM", vwap_anchor=anchor)
        b = noise_bands(d, 90)
        print(f"YM anchor={anchor}: bars={len(d)} sessions={d['sdate'].nunique()} "
