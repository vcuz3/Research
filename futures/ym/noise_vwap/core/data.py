"""
Load YM (E-mini Dow, CBOT) 1-minute bars, place the RTH cash session, and build
the per-session RTH VWAP and the same-time-of-day "Noise Area" bands used by the
...
Session-hours note:
  YM (E-mini Dow) is an equity index future, so the 09:30..16:00 ET equity cash
  session used by the NQ/ES/GC replication is YM's NATIVE most-liquid window --
  unlike gold, there is no floor-hours caveat here. We keep the identical
  09:30..16:00 ET window and the identical clock/bands/engine and swap only the
...
# Shared instrument data lives at the common YM parent (futures/ym/data), per
# RESEARCH_WORKFLOW: reusable market data is stored once at the instrument parent,
# not duplicated per project. parents[2] = futures/ym.
DATA = Path(__file__).resolve().parents[2] / "data"
PATHS = {
    "YM": DATA / "YM_1m_clean.parquet",
}

RTH_START = 9 * 60 + 30   # 09:30 ET
RTH_END = 16 * 60         # 16:00 ET

# E-mini Dow (YM, CBOT): $5 x index, min tick 1.0 index point = $5, 1 point = $5.
TICK = {"YM": 1.0}
TICK_VALUE = {"YM": 5.0}        # $ / tick / contract
POINT_VALUE = {"YM": 5.0}       # $ / point / contract
