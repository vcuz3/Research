# RTY Noise Area VWAP Project Guide

Model-neutral operating manual for the RTY (E-mini Russell 2000) port of the Noise
Area + VWAP intraday-momentum baseline. The parent project is
`futures/nq/noise_vwap` (and the GC port `futures/gc/noise_vwap`); this project
reuses the audited engine and method verbatim and swaps only the data and contract
economics.
...
| Clean data build | `core/build_clean.py` → `../data/RTY_1m_clean.parquet` |
...
E-mini Russell 2000 (RTY, CME): $50 × index, tick 0.10 pt = $5, 1 point = $50. Set
in `core/data.py` and `core/session.py` (`TICK`, `TICK_VALUE`, `POINT_VALUE`).

## Core commands

```powershell
python -m futures.rty.noise_vwap.core.build_clean
python -m futures.rty.noise_vwap.scripts.data_quality RTY 90
python -m futures.rty.noise_vwap.scripts.run_baseline RTY 90
```

## Data conventions

- Databento `RTY.v.0` raw (unadjusted) continuous 1m is the faithful source. Raw,
...
- **Sample begins 2017-07** (RTY migrated to CME July 2017) — shorter than NQ/ES/YM.
- Session = equity RTH 09:30–16:00 ET, RTY's native cash session.
