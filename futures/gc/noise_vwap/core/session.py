"""
GC (COMEX Gold) session loader for the Noise-Area + VWAP momentum grid studies.

Faithful port of `futures/nq/noise_vwap/core/session.py`; only the data path and
contract economics differ (project convention: keep GC in parity with the audited
NQ code, port fixes from NQ).
...
# COMEX Gold (GC): 100 troy oz, min tick 0.10 = $10, 1 point = $100.
TICK = {"GC": 0.10}
...
if __name__ == "__main__":
    for anchor in ("rth", "eth"):
        d = load_rth("GC", vwap_anchor=anchor)
        b = noise_bands(d, 90)
        print(f"GC anchor={anchor}: bars={len(d)} sessions={d['sdate'].nunique()} "
