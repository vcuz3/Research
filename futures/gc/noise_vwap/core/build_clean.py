"""
Build a clean 1-minute parquet dataset for GC (COMEX Gold) from the Databento
continuous-futures file in `futures/data/databento/`.

This mirrors the NQ/ES pipeline (`futures/nq/noise_vwap/core/build_clean.py`)
verbatim in method; only the instrument and source path differ.

We use the RAW (unadjusted continuous, GC.v.0) file, not the back-adjusted one:
...
Written to futures/gc/noise_vwap/data/GC_1m_clean.parquet
Run:  python -m futures.gc.noise_vwap.core.build_clean
"""
...
RAW = {
    "GC": DB / "GC_ohlcv-1m_GCv0_20110801_20260717.parquet",
}
