"""
Build clean 1-minute parquet datasets for NQ and ES from the Databento continuous
futures files in `futures/data/databento/`.

Why this replaces the old xlsx build: the old vendor ("Nearest" xlsx) series had
(a) a recent-years timezone/session-alignment defect and (b) intraday gaps that
produced fake PnL spikes. Databento ships a true UTC `ts_event`, so the ET session
clock is unambiguous and DST-correct with no lossy Chicago round-trip.

We use the RAW (unadjusted continuous, NQ.v.0 / ES.v.0) file, not the back-adjusted
one:
  * The strategy's economics are intraday percentage moves (close/open-1) and the
    session VWAP, both computed WITHIN a single RTH session. Rolls happen at UTC
    midnight (between RTH sessions), so no session/VWAP/trade spans a roll, and the
    raw within-session move is the real contract's true move.
  * Additive back-adjustment shifts every historical price by a constant, which
    distorts intraday ratios (close/open-1). Ratio distortion is exactly what the
    Noise-Area bands are built on, so raw is the faithful choice.
  * The only cross-session raw artifact is a ~1-tick roll gap in `prior_close` /
    daily close-to-close return on the ~60 roll days over 15y — immaterial.
